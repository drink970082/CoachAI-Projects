...
class BadmintonDataset(Dataset):
    def __init__(self, matches, config):
        ...
        group = matches[['rally_id', 'ball_round', 'type', 'landing_x', 'landing_y', 'player', 'set',\
        ...
        for i, rally_id in enumerate(group.index):
            ...
            time = [np.int32((datetime.strptime(time[i+1], '%H:%M:%S') - datetime.strptime(time[i], '%H:%M:%S')).total_seconds()) for i in range(len(time)-1)]
