...
def prepare_dataset(config):
    ...
    """ modified(no test_given) """
    matches = pd.read_csv(f"{config['data_folder']}train.csv")
    split_len = int(len(matches)*0.7)
    train_matches = matches[:split_len]
    val_matches = pd.read_csv(f"{config['data_folder']}val_given.csv")
    test_matches = matches[split_len:]
    """"""
