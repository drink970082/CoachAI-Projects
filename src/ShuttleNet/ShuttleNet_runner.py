...
def shotGen_trainer(data_loader, feature_name, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                    config, device="cpu"):
...
def shotgen_generator(given_seq, encoder, decoder, config, samples, device):
    ...
    with torch.no_grad():
        ...
        for sample_id in range(samples):
            ...
            generated_shot_logits.append(current_generated_shot), generated_area_coordinates.append(
                current_generated_area)
